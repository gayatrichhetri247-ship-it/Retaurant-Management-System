...
const Reservations = () => {
  ...
  const reservationProfiles = {
    'dine-in': {
      ...
      description: 'Standard floor table assignment for walk-ins, daily lunch, or dinner bookings.',
      capacity: '1 - 6 Guests',
      icon: <BiUser size={22} />,
    ...
    'family': {
      ...
      description: 'Configured clusters utilizing extended layouts. Complimentary high-chairs included.',
    ...
    'party': {
      ...
      description: 'Lively group arrangements optimized for birthdays, structural events, and hosting.',
    ...
    'corporate': {
      ...
      description: 'Quiet peripheral placements suited for business networking, lunches, or corporate dinners.',
    ...
    'vip': {
      ...
      description: 'Private booth enclosures, custom tailored menus, and structural chef integration privileges.',
      capacity: '1 - 8 Guests',
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      setSubmitted(false);
      setFormData({ guestName: '', phone: '', email: '', guestCount: '2', date: '', time: '18:00', specialNotes: '' });
    }, 4000);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="relative border-b border-slate-200 pb-8 mb-12">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">
            Reserve a <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-600">Table</span>
          </h1>
          <p className="mt-3 text-sm text-slate-500 max-w-2xl leading-relaxed">
Select your preferred dining profile below to route your configuration to our hosting registry. Live verification will adjust structural floor layouts automatically.
          </p>
        </div>
              ...
              {Object.entries(reservationProfiles).map(([key, profile]) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      boxShadow: isSelected ? `0 10px 25px -5px ${profile.glowColor}` : ''
                    }}
                  >
                    {/* Selected Active Border Glow Accent */}
                    {isSelected && (
                      <div className={`absolute inset-y-0 left-0 w-1 bg-gradient-to-b ${profile.gradient} rounded-l-xl`} />
                    )}
                    ...
                    <div className="flex items-center space-x-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className={`text-sm font-bold tracking-wide transition-colors ${isSelected ? 'text-slate-900' : 'text-slate-700'}`}>
                          {profile.title}
                        </p>
                    ...
                    <div className="flex items-center space-x-2 pl-2 flex-shrink-0">
                      ...
                      <BiChevronRight 
                        ...
                        className={`transition-transform duration-300 ${
                          isSelected ? 'text-slate-500 translate-x-0' : 'text-slate-400 -translate-x-1 group-hover:translate-x-0 group-hover:text-slate-600'
                        }`} 
                      />
                    </div>
                  </button>
          ...
          <div className="lg:col-span-7 bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 relative shadow-sm">
            {submitted ? (
              <div className="py-24 text-center space-y-4 animate-fadeIn">
                <div className={`mx-auto w-14 h-14 rounded-2xl bg-gradient-to-br ${currentProfile.gradient} text-white flex items-center justify-center shadow-md`}>
                  <BiCheckCircle size={30} />
                </div>
                <div className="space-y-1">
                  <h3 className="text-lg font-bold text-slate-900 tracking-wide">Allocation Confirmed</h3>
                  <p className="text-xs text-slate-500 max-w-sm mx-auto leading-relaxed">
                    System payload pushed successfully to central server networks. Registry tables have updated state logs.
                  </p>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className={`relative overflow-hidden rounded-xl border p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${currentProfile.bannerBg}`}>
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-lg bg-gradient-to-br ${currentProfile.gradient} text-white shadow-sm`}>
                      {currentProfile.icon}
                    </div>
                    <div>
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Active Configuration Template</span>
                      <h4 className="text-sm font-bold">{currentProfile.title}</h4>
                    </div>
                  </div>
                  <div className="sm:text-right border-t sm:border-t-0 border-slate-200/60 pt-2 sm:pt-0">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Structural Limit</span>
                    <span className="text-xs font-mono font-bold text-slate-700">{currentProfile.capacity}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Lead Representative</label>
                    <input 
                      type="text" 
                      required
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Contact Number</label>
                    <input 
                      type="tel" 
                      required
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Representative Email</label>
                  <input 
                    type="email" 
                    required
...
/>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Headcount Input</label>
                    <input 
                      type="number" 
                      required
                      min="1"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Operational Execution Notes</label>
                  <textarea 
                    rows="3"
